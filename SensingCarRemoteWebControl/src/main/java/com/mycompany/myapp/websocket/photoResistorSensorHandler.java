...
public class photoResistorSensorHandler extends TextWebSocketHandler implements ApplicationListener{
	...
	public void init() {
		...
		coapClient.setURI("coap://192.168.3.46:5683/photoresistor");
		coapObserveRelation = coapClient.observe(new CoapHandler() {
			...
			public void onLoad(CoapResponse response) {
				...
				double value = Double.parseDouble(jsonObject.getString("photoresistor"));
				jsonObject = new JSONObject();
				jsonObject.put("time", getTimeUTC(new Date().getTime()));
				jsonObject.put("photoresistor",value);
